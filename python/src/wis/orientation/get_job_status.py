...
from .orientation_result import WISOrientationResult
# #############################################

def get_job_status(token: Token, job_id: str) -> WISOrientationResult:
    ...
    """
    url = wis_service_base_url + "/orientation?orientation_job_id=" + job_id
    headers = { **token.header }
    response = requests.request("GET", url, headers=headers)
    if response.status_code == 200:
        content = response.json()
        if content['status'] != 'Unknown Job':
            return WISOrientationResult.from_content(content)
        else:
